...
[{block name="email_plain_order_cust_voucherdiscount_top"}][{if $oViewConf->getShowVouchers() }][{foreach from=$order->getVoucherList() item=voucher}]
...
[{foreach key=basketindex from=$basket->getContents() item=basketitem}]
    [{block name="email_plain_order_cust_basketitem"}]
        ...
        [{ if $basketitem->getChosenSelList() }][{foreach from=$basketitem->getChosenSelList() item=oList}]
            ...
            => [{ $oList->name }] [{ $oList->value }][{/foreach}][{/if}]
        [{ if $basketitem->getPersParams() }]
            [{ foreach key=sVar from=$basketitem->getPersParams() item=aParam }]

                => [{ $sVar }]: [{ $aParam }][{/foreach }][{/if }]
...
[{if !$basket->getDiscounts()}]
    [{block name="email_plain_order_cust_nodiscounttotalnet"}]
        [{"TOTAL_NET"|oxmultilangassign|colon|str_pad:30:" "}][{ $basket->getProductsNetPrice() }] [{ $currency->name}][{* netto price *}]
    [{/block}][{block name="email_plain_order_cust_nodiscountproductvats"}]
    [{foreach from=$basket->getProductVats() item=VATitem key=key}]
        [{"VAT_PLUS_PERCENT_AMOUNT"|oxmultilangassign:$key|colon|str_pad:32:" "}][{ $VATitem }] [{ $currency->name}][{* VATs *}]
    [{/foreach}]
[{/block}][{block name="email_plain_order_cust_nodiscounttotalgross"}]
    [{"TOTAL_GROSS"|oxmultilangassign|colon|str_pad:30:" "}][{ $basket->getFProductsPrice() }] [{ $currency->name}]
[{/block}][{* brutto price *}]
[{else}]
    [{if $order->isNettoMode() }]
        [{block name="email_plain_order_ownerdiscounttotalnet"}]
            [{ oxmultilang ident="TOTAL_NET" suffix="COLON" }] [{ $basket->getProductsNetPrice() }] [{ $currency->name}][{* netto price *}]
        [{/block}]
    [{else}]
        [{block name="email_plain_order_discountownertotalgross"}]
            [{ oxmultilang ident="TOTAL_GROSS" suffix="COLON" }] [{ $basket->getFProductsPrice() }] [{ $currency->name}][{* brutto price *}]
        [{/block}]
    [{/if}]
...
[{block name="email_plain_order_cust_voucherdiscount"}]
    ...
    [{/if}]
[{/block}][{block name="email_plain_order_cust_delcosts"}]
    [{if $basket->getDelCostNet() }]
        [{ oxmultilang ident="SHIPPING_NET" suffix="COLON" }] [{ $basket->getDelCostNet() }] [{ $currency->sign}][{* delivery costs *}]
    ...
    [{elseif $basket->getFDeliveryCosts() }][{ oxmultilang ident="SHIPPING_COST" }] [{ $basket->getFDeliveryCosts() }] [{ $currency->sign}][{/if}]
[{/block}][{block name="email_plain_order_cust_paymentcosts"}][{* payment sum *}]
    [{ if $basket->getPayCostNet() }][{if $basket->getPaymentCosts() >= 0}][{ oxmultilang ident="SURCHARGE" }][{else}][{ oxmultilang ident="DEDUCTION" }][{/if}] [{ oxmultilang ident="PAYMENT_METHOD" suffix="COLON" }] [{ $basket->getPayCostNet() }] [{ $currency->sign}][{if $basket->getPayCostVat() }]
        [{if $basket->isProportionalCalculationOn() }] [{ oxmultilang ident="BASKET_TOTAL_PLUS_PROPORTIONAL_VAT" }][{else}] [{ oxmultilang ident="VAT_PLUS_PERCENT_AMOUNT" suffix="COLON" args=$basket->getPayCostVatPercent()}][{/if}] [{ $basket->getPayCostVat() }] [{ $currency->sign}][{/if}]
    [{elseif $basket->getFPaymentCosts()}][{ oxmultilang ident="SURCHARGE" }] [{ $basket->getFPaymentCosts() }] [{ $currency->sign}][{/if}]
[{/block}]
...
[{block name="email_plain_order_cust_paymentinfo"}]
    [{if $payment->oxuserpayments__oxpaymentsid->value == "oxidpayadvance"}]
        [{ oxmultilang ident="BANK" suffix="COLON" }] [{$shop->oxshops__oxbankname->getRawValue()}]
        <br>
        [{ oxmultilang ident="BANK_CODE" suffix="COLON" }] [{$shop->oxshops__oxbankcode->value}]
        <br>
        [{ oxmultilang ident="BANK_ACCOUNT_NUMBER" suffix="COLON" }] [{$shop->oxshops__oxbanknumber->value}]
        <br>
        [{ oxmultilang ident="BIC" suffix="COLON" }] [{$shop->oxshops__oxbiccode->value}]
        <br>
