      ...
      [{block name="account_menu"}]
         ...
         [{if $oViewConf->getShowCompareList()}]
            <li class="list-group-item[{if $active_link == "compare"}] active[{/if}]">
               <a href="[{oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=compare"}]" title="[{oxmultilang ident="MY_PRODUCT_COMPARISON"}]">[{oxmultilang ident="MY_PRODUCT_COMPARISON"}][{if $oView->getCompareItemCount() > 0}]
                     <span class="badge">[{$oView->getCompareItemCount()}]</span>[{/if}]</a>
            </li>
         [{/if}]
         [{if $oViewConf->getViewThemeParam('blShowNoticelist')}]
         <li class="list-group-item[{if $active_link == "noticelist"}] active[{/if}]">
            <a href="[{oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=account_noticelist"}]" title="[{oxmultilang ident="MY_WISH_LIST"}]">[{oxmultilang ident="MY_WISH_LIST"}][{if $oxcmp_user->getNoticeListArtCnt() > 0}]
                  <span class="badge">[{$oxcmp_user->getNoticeListArtCnt()}]</span>[{/if}]</a>
         </li>
         [{/if}]
         [{if $oViewConf->getShowWishlist()}]
            <li class="list-group-item[{if $active_link == "wishlist"}] active[{/if}]">
               <a href="[{oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=account_wishlist"}]" title="[{oxmultilang ident="MY_GIFT_REGISTRY"}]">[{oxmultilang ident="MY_GIFT_REGISTRY"}][{if $oxcmp_user->getWishListArtCnt() > 0}]
                     <span class="badge">[{$oxcmp_user->getWishListArtCnt()}]</span>[{/if}]</a>
            </li>
         [{/if}]
