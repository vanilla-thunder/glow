   ...
   <div class="row">
      ...
      <div class="col-xs-12 col-sm-7" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
         ...
         <form class="js-oxProductForm form-horizontal" action="[{$oViewConf->getSelfActionLink()}]" method="post">
            ...
            <div class="priceinfo [{if $tprice && $tprice->getBruttoPrice() > $price->getBruttoPrice()}]reduced[{/if}]">
            ...
            [{block name="details_productmain_tobasket"}]
               [{if !$oDetailsProduct->isNotBuyable()}]
                  ...
                  [{else}]
                     <div class="alert alert-warning">[{oxmultilang ident="DETAILS_CHOOSEVARIANT"}]</div>
                  [{/if}]
               [{/if}]
            [{/block}]
