                           ...
                           <div class="wrapping">
                              ...
                              [{else}]
                                 ...
                                 [{if $editable}]
                                    ...
                                    <a class="btn btn-default btn-xs" href="#" title="[{oxmultilang ident="ADD_WRAPPING"}]" data-toggle="modal" data-target="#giftoptions">
                                       <i class="fa fa-pencil"></i> [{$oWrap->oxwrapping__oxname->value}]</a>
                                 [{else}]
               ...
               <div class="col-xs-5 col-sm-2 col-md-pull-3">
                           ...
                           [{if !$basketitem->isBundle() || !$basketitem->isDiscountArticle()}]
                              <div class="input-group input-group-sm">
                                 <input id="amount_[{$smarty.foreach.basketContents.iteration}]" type="number" class="textbox form-control text-center"
                                        name="aproducts[[{$basketindex}]][am]" value="[{$basketitem->getAmount()}]" size="3" min="0" style="min-width:40px; padding: 5px; font-size: 125%;">
                                 <span class="input-group-addon">
                                    [{if $basketitem->oxarticles__oxunitname->value}][{$basketitem->oxarticles__oxunitname->value}]
                                    [{else}][{oxmultilang ident="PCS"}][{/if}]
                                 </span>
                              </div>
                              <button class="btn btn-sm btn-warning btn-block" id="updateBtn_[{$smarty.foreach.basketContents.iteration}]" type="submit" name="updateBtn" title="[{oxmultilang ident="UPDATE"}]">
                  ...
                  [{block name="checkout_basketcontents_basketitem_removecheckbox"}]
                     [{if $editable}]
                        ...
                        <button type="submit" name="removeBtn" class="btn btn-sm btn-danger btn-block" onclick="document.getElementById( 'aproducts_[{$basketindex}]_remove' ).value = '1';">
